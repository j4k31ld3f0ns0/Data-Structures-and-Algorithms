...
public:
	...
	int size(); //returns value of index
	void push(const T& newItem); //checks if full, otherwise inserts value into array at correct location
	void pop(); //index - 1, doesn't actually pop item off.
	T top(); //returns top item on stack, if empty throws std::out_of_range
	void popThirdFromTop();
	void pushTwoUnderTop(const T& newItem); //same as push but under top two.
	T topThirdFromTop(); //retrieves value of item under top two, throws std::out_of_range if it isn't possible.
...
template<typename T>
int StackForCS2420<T>::size() {
	return index;
}

template<typename T>
void StackForCS2420<T>::push(const T& newItem)
{
	if (index >= capacity) {
...
T StackForCS2420<T>::top()
{
	if (index > 0) {
		...
		throw std::out_of_range("Cannot complete, no items populate stack!");
	}
}
...
void StackForCS2420<T>::popThirdFromTop()
{
	if (index > 3) {
		...
		cout << "Stack not big enough" << endl;
	}
}

template<typename T>
void StackForCS2420<T>::pushTwoUnderTop(const T& newItem)
{
	if (index < 2) { //pushing two under would go out of bounds
		cout << "Not enough items in stack!" << endl;
	}
	else if (capacity < index) { //adding additional item would go out of bounds
...
{
	if (index < 3) {
		throw std::out_of_range("Array has less than 3 items");
	}
	else {
		return arr[index - 3];
	}
...
template<typename T>
T& StackForCS2420<T>::operator[](const unsigned int index)
{
	if (index >= capacity) {
