...
//************************************************************
// Put Iterator consrtuctors and methods below here
//************************************************************
...
bool Iterator<T>::operator!=(const Iterator<T>& rhs) const
{
	...
	return (this->hashTable != rhs.hashTable || this->index != rhs.index); //<- this is smart and working
	//if (this->hashTable == rhs.hashTable) { //same HashTable address <- this is stupid and working
	//	if (this->index == rhs.index) { // same HashTable AND index addresses, FALSE!
	//		return false;
	//	}
	//	else { //not same index Address
	//		return true;
	//	}
	//}
	//else { //not same hashTable address, return TRUE
	//	return true;
	//}
}

template<typename T>
Iterator<T> Iterator<T>::operator++() //something wrong here?
{
	while (true) {
		index++;
		if (this->index == hashTable->capacity) {
			cout << "Reached end of array, cannot iterate further." << endl;
			return *this;
		}
		if (this->hashTable->status_arr[index] == 1) {
			return *this;
		}
	}
}
...
//************************************************************
// Put Iterator consrtuctors and methods above here
//************************************************************
...
public:
	...
	T& operator[](const string& key); // return by reference which can allow for modification of val-ues
...
HashTable<T>::HashTable()
{
	...
	this->keyValue_arr = make_unique<pair<string, T>[]>(capacity);

	//set all status array elements to zero using for loop
...
template<typename T>
HashTable<T>::HashTable(const unsigned int capacity) {
...
template<typename T>
HashTable<T>& HashTable<T>::operator=(HashTable<T>&& objToMove) noexcept //move semantics baybee
{
	// TODO: insert return statement here
	if (this != &objToMove) {
		this->status_arr = move(objToMove.status_arr); //std::move arrays, complex data types
		this->keyValue_arr = move(objToMove.keyValue_arr);
		this->capacity = objToMove.capacity; //atomic data types, no need for move semantics.
	...
	else { //wtf?? same object?? bad dog!
		cout << "Same object, can not move." << endl;
		return *this;
	}
}

template<typename T>
void HashTable<T>::create(const string& key, const T& item) //L value create, named. Copy stuffs
{
	double percentFilled = (count + 1.0) / capacity;
	if (percentFilled > .75) { //check if rehash is necessary
		HashTable<T>* newHash = new HashTable<T>(capacity * 2);

		for (unsigned int i = 0; i < this->capacity; i++) { //iterate Capacity times.
			if (this->status_arr[i] == 1) { //if the old status arr has an item, use create
				//if found, pass in old key, and std move the old value
				newHash->create(this->keyValue_arr[i].first, this->keyValue_arr[i].second);
			}
		}
		*this = std::move(*newHash);
	}
	//rehash not necessary, just create
	unsigned int index = hash(key);
	bool keepIterating = true;
	do {
		if (status_arr[index] != 1) { //empty!
			this->keyValue_arr[index].first = key;
			this->keyValue_arr[index].second = item;
			this->status_arr[index] = 1;
			this->count++;
			keepIterating = false;
		}
		else {// cringe, not empty.
			if (index == capacity - 1) { //if at MAX - 1 because arrays are 0 based.
				index = 0; //set to 0 because if you iterate further it will be out of range
			}
			else { //if not at the end, simply iterate
				index++;
			}
		}
	} while (keepIterating);
...
template<typename T>
void HashTable<T>::create(const string& key, T&& item) //R value create, move stuffs
{
	double percentFilled = (count + 1.0) / capacity;
	if (percentFilled > .75) { //check if rehash is necessary
		HashTable<T>* newHash = new HashTable<T>(capacity * 2);
		...
		for ( unsigned int i = 0; i < this->capacity; i++) { //iterate Capacity times.
			if (this->status_arr[i] == 1) { //if the old status arr has an item, use create
				//if found, pass in old key, and std move the old value
				newHash->create(this->keyValue_arr[i].first, std::move(this->keyValue_arr[i].second));
			}
		}
		*this = std::move(*newHash);
	}
	...
	do {
		if (status_arr[index] != 1) { //empty!
			this->keyValue_arr[index].first = std::move(key);
			this->keyValue_arr[index].second = std::move(item);
			this->status_arr[index] = 1;
			this->count++;
			keepIterating = false;
		}
...
T HashTable<T>::retrieve(const string& key)
{
	int index = hash(key);
	int i = index;
	int itemsNotChecked = this->count;
	while (itemsNotChecked > 0) {
		if (this->status_arr[i] == 1) { //current index is populated by *something*
			if (this->keyValue_arr[i].first == key) { //check if it matches key
				return keyValue_arr[i].second; //returning data.. Hope it doesnt want the pair.
			}
			else { //it found a populated item, but it wasn't the right one.
				itemsNotChecked--;
			}
		}
		if (i == capacity - 1) { //if the current index is at the end of the array, reset and go to beginning
			i = 0;
		}
		else {
			i++;
	...
	throw std::logic_error("Error: key not found."); //went through whole array and found nothing
}
...
bool HashTable<T>::exists(const string& key)
{
	...
	return false; //went through whole array and found nothing
}
...
{
	int index = hash(key);
	int i = index; //so I can manipulate the index
	int itemsNotChecked = this->count; // all items
	while (itemsNotChecked > 0) {
		if (this->status_arr[i] == 1) { //something is here, check it!
			if (this->keyValue_arr[i].first == key) { //key matches, this is the one!
				this->status_arr[i] = -1; //mark as removed
		...
		if (i == capacity - 1) { //the index is at the end of the array
			i = 0;
		}
		else { //not at end of array, keep iterating
			i++;
		}
	} //end of checking all items
	cout << "Checked entire array, item not found." << endl;
}

template<typename T>
Iterator<T> HashTable<T>::begin()
{
	Iterator<T> theIterator; //iterator to return
	theIterator.hashTable = this; //hash table address
	unsigned int index = 0;
	while (index < this->capacity) { //start at zero and go to cap - 1
		if (this->status_arr[index] == 1) { //if it found an item, stop at that item and return
			theIterator.index = index;
			return theIterator;
		}
		index++; //otherwise keep iterating until it finds something
	}
	theIterator.index = this->capacity; //nothing in the array!!!
	return theIterator;
}
...
Iterator<T> HashTable<T>::end()
{
	...
	theIterator.hashTable = this;
	theIterator.index = this->capacity;
	return theIterator;
}
