...
public:
	QueueForCS2420(const unsigned int capacity); //constructor
	
	unsigned int size();
	void push_back(const T& newItem); 
	void pop_front(); 
	const T& front(); //return head
	const T& back(); //return tail
	void resize(const unsigned int newCapacity); //copy and paste items in array into new different sized array
...
private:
	...
	unsigned int sizeInUse{0}; //number of slots currently in use
	//think of these two below as a snake, start at the head and go to the tail
	unsigned int head{0}; //front of queue - the oldest item in array
	unsigned int tail{0}; //back of queue - the newest item in array
...
QueueForCS2420<T>::QueueForCS2420(const unsigned int capacity) {
	...
	this->arr = new T[capacity]; //initialize the array from pointer
}
...
template<typename T>
void QueueForCS2420<T>::push_back(const T& newItem) //add item
{
	if (sizeInUse == capacity) {
		cout << "Cannot push back, Queue is full." << endl;
		return; //exit
	}

	if (tail == capacity) { //end of array, reset position
		tail = 0;
	}
	arr[tail] = newItem;
	tail++;
...
template<typename T>
void QueueForCS2420<T>::pop_front() //remove item
{
	if (sizeInUse < 1) {
		cout << "Cannot pop, no items in queue." << endl;
		return;
	}
	head++;
	if (head == capacity) { //end of array, reset position
		head = 0;
	}
	sizeInUse--;
}
...
template<typename T>
const T& QueueForCS2420<T>::front()
{
	if (sizeInUse < 1) {
...
const T& QueueForCS2420<T>::back()
{
	...
	if (sizeInUse < 1) {
		...
		return arr[tail - 1]; //NEWEST item at the back of the line
	}
}

template<typename T>
void QueueForCS2420<T>::resize(const unsigned int newCapacity)
{
	QueueForCS2420<T>* newArr = new QueueForCS2420(newCapacity); //create new array
	int actualSize = 0;

	while (this->sizeInUse > 0) {//***copy data into new array***
		T data = this->front(); //retrieve oldest data
		newArr->push_back(data); //insert data into new array
		this->pop_front(); //delete oldest data
	}
	delete[] this->arr;
	this->arr = newArr->arr; //set This's address to address of new array made by NewArr.
	newArr->arr = nullptr; //gets rid of newArr's ptr so that only This's pointer points to that data.
	this->capacity = newArr->capacity;
	this->sizeInUse = newArr->sizeInUse;
	this->head = newArr->head; //since the relative position of elements has reset, also need to reset the head/beginning to reflect that change
	this->tail = newArr->tail;
}

template<typename T>
T& QueueForCS2420<T>::operator[](const unsigned int index)
{
	if (index >= capacity) {
		cout << "The index was out of bounds" << endl;
		throw std::out_of_range("The index was out of bounds");
	}
	else {
		return arr[index];
	}
...
void testDestructor() {
...

	{
		...
		dQueue.resize(5); //this method somehow messes with the destructor
	}  // <--- Destructor should invoke here.
